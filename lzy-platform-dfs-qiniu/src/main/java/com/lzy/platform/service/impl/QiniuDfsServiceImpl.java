...
/**
 * <p>
 * 七牛云 分布式文件存储 Service 实现类
 * </p>
 *
 * @author lzy
 * @since 2023/8/3 11:07
 */
@Slf4j
@AllArgsConstructor
public class QiniuDfsServiceImpl implements IDfsBaseService {
    ...
    public LzyCloudDfsFile uploadFile(InputStream inputStream, String bucket, String fileName) {
        ...
        try {
            ...
            //解析上传成功的结果
            dfsFile = JsonUtils.jsonToPojo(response.bodyString(), LzyCloudDfsFile.class);
            if (dfsFile != null) {
    ...
    public String removeFiles(String bucket, List<String> fileNames) {
        String resultStr = null;
        try {
            if (!CollectionUtils.isEmpty(fileNames) && fileNames.size() > LzyCloudConstant.Number.THOUSAND)
            {
