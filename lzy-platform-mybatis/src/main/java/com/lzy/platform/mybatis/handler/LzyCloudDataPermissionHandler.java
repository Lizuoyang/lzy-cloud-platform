...
import com.lzy.platform.base.annotation.auth.DataPermission;
...
import com.lzy.platform.mybatis.enums.DataPermissionTypeEnum;
...
public class LzyCloudDataPermissionHandler implements DataPermissionHandler {
    ...
    public void processDataPermission(PlainSelect plainSelect, String mappedStatementId) {
        try {
            LzyCloudUser loginUser = LzyCloudAuthUtils.getCurrentUser();
            ...
            if (ObjectUtils.isNotEmpty(loginUser) && CollectionUtils.isNotEmpty(loginUser.getDataPermissionTypeList())) {
                ...
                {
                    // 2 根据注解的数据权限拼装sql
                    Class<?> clazz = Class.forName(mappedStatementId.substring(LzyCloudConstant.Number.ZERO, mappedStatementId.lastIndexOf(StringPool.DOT)));
                    String methodName = mappedStatementId.substring(mappedStatementId.lastIndexOf(StringPool.DOT) + LzyCloudConstant.Number.ONE);
     ...
     * @return 构建后查询条件
     */
    public static void dataPermissionFilter(LzyCloudUser user, DataPermissionEntity dataPermissionEntity, PlainSelect plainSelect) {
